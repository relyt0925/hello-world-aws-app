...
git clone https://github.com/relyt0925/hello-world-aws-app.git
cd hello-world-aws-app
export GIT_HEAD_COMMIT=$(git rev-parse HEAD)
echo "$SIGNING_PRIV_KEY_64" | base64 -d > /tmp/priv.key
gpg --import /tmp/priv.key
rm -f /tmp/priv.key
export PIPELINE_IMAGE_URL="us.icr.io/tylerlisnamespace/hello-world-aws-app:${GIT_HEAD_COMMIT}"
export FULL_IMAGE_TAG="$PIPELINE_IMAGE_URL"
make dockerbuild
envsubst < deployment/deployment.yaml > deployment/templated-deployment.yaml
ibmcloud sat config version create --name "$GIT_HEAD_COMMIT" --config hello-world-aws-app --file-format yaml --read-config deployment/templated-deployment.yaml
